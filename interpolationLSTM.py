...
if __name__ == '__main__':
    ...
    scaler.fit_transform(remain_data)
    ...
    r = kalmanData['remain_perc'].loc[startDate:].to_numpy(dtype=float)
    P0_r = r.var()
    H = 1
    R_r = 13.08
    P0_r = 10
    ...
    for i in range(tPolls + 5):
        xf = model.predict(seq)
        seq = seq.flatten()
        seq = np.append(seq, xf[0][0])
        seq = seq[1:]
        seq = np.reshape(seq, (1, length, 1))
        preds_extra_remain.append(scaler.inverse_transform(xf)[0][0])
    ...
    #h_agg.index -= dt.timedelta(days=tTwitter)  # make h_agg date index normal again
